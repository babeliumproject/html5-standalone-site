...
<div id="registerError">
	<strong>ERROREA</strong>: {$errors}
</div>
{/if}
{if $success}
<div id="registerSuccess" class="sameMargin">
	<h2>Erregistro egokia. Konfirmaziorako posta elektroniko bat bidali da eman duzun helbidera.</h2>
</div>
{else}
<div id="registerForm" class="sameMargin">
	<form onsubmit="new RegisterEvent(RegisterEvent.REGISTER_USER, new NewUserVO(this.username.value, Sha1.hash(this.password.value, true), this.realName.value, this.realSurname.value, this.email.value, '')).dispatch(); return false;">
		<fieldset>
			...
			Erabiltzaile izena: <input type="text" name="username" required> <br>
			Posta elektronikoa: <input type="email" name="email" required placeholder="Balizko posta helbide elektroniko bat sartu"> <br>
			Pasahitza: <input type="password" name="password" required> <br>
			Pasahitza berriz sartu: <input type="password" name="repassword" required> <br>
			Zure Izena: <input type="text" name="realName"> <br>
			Zure Abizena: <input type="text" name="realSurname">
		</fieldset>
		<fieldset>
